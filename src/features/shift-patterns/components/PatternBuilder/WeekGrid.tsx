      ...
      {isRestDay ? (
        ...
        <div className="space-y-1">
          ...
          {dayData.breakMinutes && dayData.breakMinutes > 0 && (
            <div className={`text-xs ${styles.text} opacity-60`}>{dayData.breakMinutes}m break</div>
          )}
        </div>
      ) : (
...
export function WeekGrid({
  ...
  const getDayData = useCallback(
    (dayOfWeek: DayOfWeek): PatternDayFormData | undefined => {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        {rotationCycleWeeks > 1 && onCopyWeek && (
          <div className="relative" ref={copyWeekRef}>
            ...
            >
              ...
              Copy to Week
              <ChevronDown
                className={`h-4 w-4 transition-transform ${showCopyWeekMenu ? 'rotate-180' : ''}`}
              />
            </button>
      ...
      <ContextMenu
        ...
        isCurrentlyRestDay={
          contextMenu.dayOfWeek ? (getDayData(contextMenu.dayOfWeek)?.isRestDay ?? true) : false
        }
      />
