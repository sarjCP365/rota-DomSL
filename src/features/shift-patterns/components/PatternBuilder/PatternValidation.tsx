  ...
  const validations = useMemo<ValidationMessage[]>(() => {
    ...
    for (let week = 1; week <= rotationCycleWeeks; week++) {
      for (let i = 1; i < dayOrder.length; i++) {
        ...
        const prevEndMins = timeToMinutes(prevDay.endTime);
        const currStartMins = timeToMinutes(currDay.startTime);
        ...
        {errors.map((msg) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              {msg.detail && <p className="mt-0.5 text-xs text-slate-500">{msg.detail}</p>}
            </div>
